...
// print_r($group_code);
	...
	private function renderFields($code ,$key, $field){
		...
		if($field['type'] != 'img' && $field['type'] != 'limg'){
			...
			$code = preg_replace('/\{\{' . html_entity_decode($pre_key, ENT_QUOTES, 'UTF-8') . '\|{0,1}[a-z]*\}\}/Um', $field['value'], html_entity_decode($code, ENT_QUOTES, 'UTF-8'));
			...
			if(strpos($field['value'], 'view') === false){
				...
				preg_match('/\{\{.*' . $pre_key . '.*?(class=[\'"].*[\'"]).*?\|{0,1}.*\}\}/Um', $code, $iclass);
				$code = preg_replace('/\{\{.*' . $pre_key . '.*?\|{0,1}.*\}\}/Um', '<img src="' . $field['value'] . '" width="' . $field['width']. '" height="' . $field['height']. '" alt="' . $field['alt']. '" title="' . $field['title']. '"' . ($iclass?' ' . $iclass[1]:'') .' />', $code);
			}
			...
			if(strpos($field['value'], 'view') === false){
				...
				$img = '<img src="' . $this->model_tool_image->resize($field['value'], $field['width'], $field['height']) . '" width="' . $field['width']. '" height="' . $field['height']. '" alt="' . $field['alt']. '" title="' . $field['title']. '" />';
...
// print_r($lcode3);
				...
				// print_r($lcode2[0]);
				// print_r($field);
				$img = '<img src="' . $field['value'] . '" width="' . $field['width']. '" height="' . $field['height']. '" alt="' . $field['alt']. '" title="' . $field['title']. '" />';
				$lcode3 = preg_replace('/<img.*>/U', $img, $lcode2[0]);
